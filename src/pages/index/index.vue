<template>
  <div @click="clickHandle" class="wrap">
    <!-- 首页地图模块 -->
    <map
      id="map"
      :longitude="location.longitude"
      :latitude="location.latitude"
      subkey="VUJBZ-6RXWJ-QBHFH-KVQDL-BDSNJ-LFBJG"
      scale="14"
      show-location
      show-compass
      :markers="markers"
      @markertap="markertap"
      @regionchange="regionChange"
    ></map>
    <!-- 重新定位图标 -->
    <cover-view class="current" @tap="goCurrent">
      <cover-image class="img" src="/static/images/location.png" />
    </cover-view>
    <cover-view>
      <cover-view class="my" @click="tap">
          <cover-image class="imge" src="/static/images/wodedangxuan (1).png"></cover-image>
      </cover-view>
    </cover-view>

    <button class="add_mianshi" @click="navAddPage">
        添加面试
    </button>

  </div>
</template>

<script>
import {getLocation, getAuth} from '@/utils/index.js'
import {mapState, mapMutations} from 'vuex'

export default {
  data () {
    return {
      // 用户当前位置
      location: {
        latitude: 40.03298,
        longitude: 116.29891
      },
      markers: [{
        iconPath: '/static/images/job.png',
        id: 0,
        latitude: 23.099994,
        longitude: 113.324520,
        width: 50,
        height: 50
      }]
    }
  },




  methods: {

    btnClick(type){
      this.changeNum(type);
    },
    regionChange(e){

    },
    // 点击标注物
    marketTap(e){

    },
    // 重新定位
    goCurrent(){
      getAuth('scope.userLocation', async ()=>{
        let location = await getLocation();
        wx.setStorageSync('location', location)
        this.location = location;
      })
    },
    tap(){
      wx.navigateTo({url:'/pages/my/main'})
    },
    navAddPage(){
      wx.navigateTo({
        url:'/pages/addpage/main'
      })
    }
  },

  created () {
    let location = wx.getStorageSync('location');
    this.location = location;
  }
}
</script>

<style lang="scss" scoped>
.wrap{
  height: 100%;
  overflow: hidden;
}
map{
  width: 100%;
  height: 100%;
}
.current{
  position: fixed;
  bottom: 100rpx;
  width: 80rpx;
  height: 80rpx;
  left: 20rpx;
}
.my{
  width:80px;
  height:45px;
  position: fixed;
  bottom:100rpx;
  right:-40px;
  border-radius:22.5px;
  background-color:	#DCDCDC;
  display:flex;
  align-items:center;
  line-height:45px;
  padding-left:10px;
}
.imge{
  width:60rpx;
  height:60rpx;
  display:block;
}
.add_mianshi{
  width:80%;
  background-color:skyblue;
  color:white;
  position:fixed;
  bottom:5px;
  left:50%;
  transform: translate(-50%)
}
</style>
